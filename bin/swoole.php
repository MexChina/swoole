...
global $_G; //定义全局变量
// $_G['server_type'] = "app";
$_G['server_type'] = "gearman";

//调试代码------------------------------
//$argv[1] = $argv[1] ? $argv[1] : "reload";
$argv[2] = !empty($argv[2]) ? $argv[2] : "PositionEtl";
//$argv[3] = "132577";
//$argv[4] = "resumes_basedata";
//$argv[5] = "10";
//$argv[6] = "50000";
//$argv[5] = "functions";
$argv[10] = "debug";
...
$cmd = !empty($argv[1]) ? $argv[1] : 'start'; //脚本控制命令
$app = !empty($argv[2]) ? $argv[2] : "ResumeEtl"; //应用标识,默认App
unset($argv[0]);
unset($argv[1]);
unset($argv[2]);
$_G["argv"] = [];
...
foreach ($argv as $value) {
    ...
    } elseif (strtolower($value) == "dev") {
        define("SWOOLE_ENVIRONMENT", "dev");
    } elseif (strtolower($value) == "test") {
        define("SWOOLE_ENVIRONMENT", "test");
    } elseif (strtolower($value) == "pro") {
        define("SWOOLE_ENVIRONMENT", "pro");
    } else {
        $count++;
        $_G["argv"][$count] = $value;
    }
...
//默认为生产环境
//dev：开发环境， test：测试环境， pro生产环境
defined("SWOOLE_ENVIRONMENT") or define("SWOOLE_ENVIRONMENT", "dev");
...
define('APP_ENV',0);    //0,1,2  开发，测试，上线
...
$master_pid_file = SWOOLE_ROOT_DIR . 'log/'.SWOOLE_APP.'/Swoole_matser_pid.pid';
