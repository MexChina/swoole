...
class DBServer {
    ...
    function run() {
        $serv = new swoole_server("127.0.0.1", 13307);
        $serv->set(array(
            'worker_num' => 1,
            'max_request' => 0,
        ));
    ...
    function onStart($serv) {
        ...
        for ($i = 0; $i < $this->pool_size; $i++) {
            $db = new mysqli;
            //$db->connect('p:127.0.0.1', 'rsdata', 'rsdata0824', 'resumes_basedata_test', '3307');
            $db->connect('p:192.168.8.109', 'kdd', 'kd12934d', 'icdc_0', '3306');
            ...
            echo "db [db_sock:{$db_sock}] create pool success.....\n";
        }
    ...
    function onSQLReady($db_sock) {
        ...
        if (!is_object($mysqli)) {
            $this->serv->send($fd, "MySQLi Error: Link disconnect......");
        } elseif ($result = $mysqli->reap_async_query()) {
            if (is_object($result)) {
