...
class Refresh extends \Swoole\Core\App\Controller{
    ...
    private $process_total=50;     //总进程数  2的幂数 db_total的倍数
    private $db_total=5;           //数据库的个数
    ...
    public function index(){
        ...
        for($page=1;$page<=$page_total;$page++){
            ...
            $result = $this->read_db->query("select id,compress from resumes_extras where id >= {$this->start_id} and id <= {$this->end_id} order by id asc limit {$this->page_size}")->fetchall();
            foreach($result as $r){
                $this->algorithm($r);
                $this->start_id = $r['id'];
            }
            $percent = sprintf("%.2f", $page/$page_total*100);
            $run = number_format(microtime(true), 8, '.', '') - $istart_time;
            $run = sprintf("%.3f", $run).'s';
            $mem = System::get_used_memory();
            Log::write_log("{$this->db_name} {$this->start_id} {$percent}% $run $mem");
        }
    ...
    private function algorithm($extra){
        ...
        if($compress['basic']['is_deleted'] == 'Y'){
            Log::write_log("$resume_id is_deleted");
            return;
        }
    ...
    public function CVFwdIndex($compress,$algorighm,$cv_workyear){
        ...
        $this->fwdindex_service_online->doBackground("fwdindex_service_online",json_encode($param));
    }
    ...
    public function EsServers($compress,$resume){
        ...
        $this->es_servers->doBackground("es_servers",json_encode($param));
    }
