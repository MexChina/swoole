...
class ApiRelationChain extends \Swoole\Core\App\Controller{
        ...
        $worker->worker(function($request) use($resume){
            ...
            if($results){
                ...
                foreach($results as $k=>$r){
                    ...
                    $new_results[$k]= is_null($value) ? array() : $value;
                }
                $results = $new_results;
            }
    ...
    public function search(){
        ...
        $worker->worker(function($request) use($resume){
            ...
            $resule_list = $resume->search($request['company_id'],$page,$page_size);
