...
class AppServer {
    ...
    function swooleSet($array_config) {
        //设置动态worker进程，可以根据实际需要动态 调整worker进程的多少, 在APP目录中的Appinit文件中的get_worker_number方法进行自定义
    ...
    function onStart($serv) {
        ...
        swoole_set_process_name("swoole" . SWOOLE_APP . ucfirst(SWOOLE_ENVIRONMENT) . "Master");
        Log::writelog("master process start ......");
    }

    function onManagerStart($serv) {
        swoole_set_process_name("swoole" . SWOOLE_APP . ucfirst(SWOOLE_ENVIRONMENT) . "Manager");
    ...
    function onWorkerStart($serv, $worker_id) {
        ...
        if ($worker_id >= $serv->setting['worker_num']) {
            swoole_set_process_name("swoole" . SWOOLE_APP . ucfirst(SWOOLE_ENVIRONMENT) . "Tasker{$worker_id}");
            Log::writelog("task worker(id:$worker_id) start ......");
            //执行task进程初始化程序
            App::init("tasker_init");
        } else {
            swoole_set_process_name("swoole" . SWOOLE_APP . ucfirst(SWOOLE_ENVIRONMENT) . "Worker{$worker_id}");
            if ($worker_id == ($serv->setting['worker_num'] - 1) && !empty(self::$config->get("server[worker_heart_time]"))) {
                //初始化监控进程
                swoole_set_process_name("swoole" . SWOOLE_APP . ucfirst(SWOOLE_ENVIRONMENT) . "Monitor{$worker_id}");
...
//        }
    ...
    function onPipeMessage($server, $from_worker_id, $data) {
        ...
        //Log::write_log("from_worker_id:$from_worker_id, data:".var_export($data, true));
        //心跳检测
        switch ($data) {
            case "~~mreq":
            ...
            case "~~mresclose":
                ...
                break;
            default:
                $is_call_back = empty($data['is_call_back']) ? false : $data['is_call_back'];
                unset($data['is_call_back']);
                $rs = App::doaction($data);
                if ($is_call_back) {
                    $back_data['params'] = $rs;
                    $back_data['app'] = $from_worker_id >= $this->swoole->setting['worker_num'] ? (SWOOLE_APP . 'Task') : SWOOLE_APP;
                    $back_data['action'] = 'sedmsg_response';
                    $back_data['task_id'] = $this->workerid;
                    $back_function_rs = $this->send_to_worker($back_data, $from_worker_id);
                }
                break;
        }
    }
    ...
    function onReceive($serv, $fd, $from_id, $data) {
        /*
         * 访问参数及路由规则
         * type=App&app=Count&action=index&params=[]
         */
...
//发送数据到另外一个进程
    protected function restowork($result, $work_id) {
        ...
        if (strlen($result) > 10240) {//数据包大于10K则需要压缩
            $result = bin2hex(gzcompress($result));
            $message['data_type'] = 1;
        }
...
//        print_r($unpack_data);
    ...
    public function reload_config($file_name) {
        return self::$config = Config::reload($file_name);
    }
