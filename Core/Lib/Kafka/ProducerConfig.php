...
class ProducerConfig extends Config
{
    ...
    public function setRequestTimeout($requestTimeout)
    {
        if (!is_numeric($requestTimeout) || $requestTimeout < 1 || $requestTimeout > 900000) {
            throw new Exception('Set request timeout value is invalid, must set it 1 .. 900000');
        }
    ...
    public function setProduceInterval($produceInterval)
    {
        if (!is_numeric($produceInterval) || $produceInterval < 1 || $produceInterval > 900000) {
            throw new Exception('Set produce interval timeout value is invalid, must set it 1 .. 900000');
        }
    ...
    public function setTimeout($timeout)
    {
        if (!is_numeric($timeout) || $timeout < 1 || $timeout > 900000) {
            throw new Exception('Set timeout value is invalid, must set it 1 .. 900000');
        }
    ...
    public function setRequiredAck($requiredAck)
    {
        if (!is_numeric($requiredAck) || $requiredAck < -1 || $requiredAck > 1000) {
            throw new Exception('Set required ack value is invalid, must set it -1 .. 1000');
        }
    ...
    {
        if (!is_bool($asyn)) {
            throw new Exception('Set isAsyn value is invalid, must set it bool value');
        }
