...
//require_once THRIFT_CLIENT . '/../Thrift/ClassLoader/ThriftClassLoader.php';
//require_once THRIFT_CLIENT . '/../Services/Hbase/Hbase.php';

use Services\Hbase\IOError;

$loader = new \Thrift\ClassLoader\ThriftClassLoader();
$loader->registerNamespace('Thrift', THRIFT_CLIENT . '/../Thrift/');
$loader->register();
...
class ThriftInstance {
    ...
    const RECONNECT_FREQUENCY = 1000;
    ...
    public function __call($method_name, $arguments) {
        ...
        // 每次都重新创建一个实例
        // 这里改成1000词重建一次
        if ($this->thriftInstance == null || ($this->call_times >= self::RECONNECT_FREQUENCY)) {
            $this->thriftInstance = null;
        ...
        while ($retry < 3) {
            try {
                ...
                $retry++;
                echo "ThriftClient Operation [" . $method_name . "] retry " . $retry . " times.\n";
                if ($retry == 3) {
                    throw $e;
                }
                usleep(1000000);
    ...
    protected function __instance() {
        $retry = AddressManager::getAddressNum($this->serviceName) + 3;
        while ($retry > 0) {
            // 获取一个服务端节点地址
