...
class Strings {
    ...
    private static $crc64_table;

    /**
     * 将IP转换成相应的数字表示
     * @param string $ip  字符串表示的ip
     * @return float
     */
    ...
    public static function crc64_hash($string) {
        if (self::$crc64_table === null) {
            self::crc64_table();
        }
        $crc = 0;
        for ($i = 0; $i < strlen($string); $i++) {
            $crc = self::$crc64_table[($crc ^ ord($string[$i])) & 0xff] ^ (($crc >> 8) & ~(0xff << 56));
        }
        return sprintf("%u", $crc);
    }

    private static function crc64_table() {
        $crc64tab = [];
        // ECMA polynomial
