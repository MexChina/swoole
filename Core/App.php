...
class App {
    ...
    public static function doaction($data) {
        ...
        //self::$app = & self::$apps[$appname];
        $task_id = empty($data['task_id']) ? 0 : $data['task_id'];
        if ($data['worker_type'] == 'task' && isset($data['task_id'])) {
            $return = call_user_func_array(array(self::$apps[$appname], "task_response"), array("params" => $params, "task_id" => $task_id));
        } elseif (method_exists(self::$apps[$appname], $appaction)) {
            $return = call_user_func_array(array(self::$apps[$appname], $appaction), array("params" => $params, "task_id" => $task_id));
        } else {
            if ($fd) {
                //AppServer::$instances->response("app:{$appname} is not exist action:{$appaction}......", $fd);
            }
